...
def startup():
    ...
    try:
        ...
        if admin_email and admin_password and not db.query(User).filter(User.email == admin_email).first():
            db.add(User(email=admin_email, password_hash=hash_password(admin_password), username="FenixAdmin", is_admin=True))
            db.commit()
        scan_music(db)
    finally:
        db.close()
...
def scan_music(db: Session):
    seen = set()
    for folder in (AUDIO_DIR, UPLOAD_DIR):
...
def tracks(request: Request, db: Session = Depends(get_db)):
    ...
    return [track_json(t, t.id in liked_ids) for t in rows]
